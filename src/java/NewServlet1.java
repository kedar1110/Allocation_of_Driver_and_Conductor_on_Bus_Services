    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter out = response.getWriter();
           try {
            ...
            try{
                if(field9.equals(field10))
                {
            Class.forName("com.mysql.jdbc.Driver");
            Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/member database","root","");
            PreparedStatement pst;
                    pst = con.prepareStatement("insert into member(MemberName,MemberType,DutyType,Licence,Addhar_Card,Address,Contact,UserName,Password,MemberStatus)values(?,?,?,?,?,?,?,?,?,?)");
